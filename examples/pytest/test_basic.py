...
def test_javascript_execution(browser, test_urls):
    ...
    assert page_info["hasLocalStorage"] == True
...
def test_error_handling(browser):
    ...
    with pytest.raises(Exception):
        page.goto("https://this-domain-definitely-does-not-exist.invalid", 
...
def test_browser_context_isolation(browser):
    ...
    # Get cookies (if any)
    cookies_before = page1.context.cookies()
    ...
    storage_value = page2.evaluate("localStorage.getItem('test-key')")
    # Note: In same profile, localStorage might persist - this depends on setup
    
    cookies_after = page2.context.cookies()
